...
class Train:
    def __init__(self, name='', num=0, arr_time='', dep_time='', src='', des='', day_of_travel='',
                 seat_available_in_1AC=0, seat_available_in_2AC=0, seat_available_in_SL=0,
                 fare_1ac=0, fare_2ac=0, fare_sl=0):
        self.name = name
        self.num = num
    ...
    def print_seat_availability(self):
        print(f"No. of seats available in 1AC: {self.seats['1AC']}")
        print(f"No. of seats available in 2AC: {self.seats['2AC']}")
        print(f"No. of seats available in SL: {self.seats['SL']}")

    def check_availability(self, coach='', ticket_num=0):
        coach = coach.upper()
        if coach not in self.seats:
            self.print_seat_availability()
            coach = input("Enter the coach (1AC/2AC/SL): ").upper()
            return self.check_availability(coach, ticket_num)
        
        if self.seats[coach] < ticket_num:
            return False
        else:
            return True

    def book_ticket(self, coach='', no_of_tickets=0):
        if self.check_availability(coach, no_of_tickets):
            self.seats[coach] -= no_of_tickets
            print(f"{no_of_tickets} ticket(s) successfully booked in {coach}.")
            return True
        else:
            print(f"Booking failed. Not enough tickets available in {coach}.")
            return False
...
t1 = Train('kakinada', 12345, '12:34', '22:12', 'ctc', 'Vijayawada', 'Wed', 30, 23, 43, 2205, 320, 234)
t2 = Train('chennai', 12565, '02:34', '23:12', 'mas', 'kol', 'Mon', 33, 4, 12, 3434, 435, 234)
t3 = Train('bangalore', 22353, '11:56', '03:12', 'ctc', 'ban', 'Fri', 33, 24, 77, 455, 325, 533)

trains = {t1.num:t1,t2.num:t2,t3.num:t3}
u1 = user(1111,'Rajesh','kakinada','8897735844','Rajesh')
u2 = user(2322,'Teddy','Nellore','8897735844','Teddy')
